...
def main(img_input, save_output=True):
    ...
    # Just one image to process now, no need for a loop
    try:
        if args.face_enhance:
            _, _, output = face_enhancer.enhance(
                img_input, has_aligned=False, only_center_face=False, paste_back=True
            )
        else:
            output, _ = upsampler.enhance(img_input, outscale=args.outscale)
        ...
        if save_output:
            if args.ext == "auto":
                extension = ".png"  # Assuming a default extension
            else:
                extension = args.ext
            if args.suffix == "":
                save_path = os.path.join(args.output, f"output.{extension}")
            else:
                save_path = os.path.join(
                    args.output, f"output_{args.suffix}.{extension}"
                )
            cv2.imwrite(save_path, output)
        else:
            outputs.append(output)

    return outputs if not save_output else None


if __name__ == "__main__":
    outputs = main(img_input=None, save_output=False)
